# optimizer class mainly built on StyleGAN-2.
import numpy as np
...
try:
    from tensorflow.python.ops import nccl_ops # for TensorFlow 1.13
except:
    import tensorflow.contrib.nccl as nccl_ops # for older TensorFlow versions

class Optimizer:
    def __init__(self,
        name:                   str             = "Train",                     # name for TensorFlow graph
        tf_optimizer:           str             = "tf.train.AdamOptimizer", # optimizer type (choose RMSProp in inference and Adam in training)
        learning_rate:          TfExpressionEx  = 0.001,                    # learning rate (overwritten in many cases)
    ...
    def register_gradients(self, loss: TfExpression, trainable_vars: Union[List, dict]) -> None:
        ...
        with tf.name_scope(self.id + "_grad"), tf.device(device.name), tf.control_dependencies(deps):
            ...
            grads, varnames = zip(*grad_list) # variables
            self.norm = norm = tf.global_norm(grads)
            clip_dep = autosummary.autosummary(self.id + "/norm", norm)
            if self.clip is not None:
                with tf.control_dependencies([clip_dep]):
